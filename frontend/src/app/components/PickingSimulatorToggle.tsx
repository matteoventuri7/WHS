...
export default function PickingSimulatorToggle() {
    ...
    useEffect(() => {
        fetch('http://localhost:3008/picking-simulator/status')
            ...
            .catch((err) => {
                console.error('Failed to get picking simulator status:', err);
                setLoading(false);
            });
    }, []);
    ...
    const toggleSimulation = async () => {
        ...
        try {
            ...
            console.error(`Failed to toggle picking simulator to ${!isSimulating}:`, err);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="flex items-center gap-4 bg-slate-900/50 px-4 py-2 rounded-xl border border-white/5">
            <div className="flex flex-col items-end">
                <span className="text-sm font-medium text-slate-300">Picking Simulator</span>
                <select
                    value={intervalMs}
            ...
            <div className="flex items-center gap-2">
                <button
                    ...
                    disabled={loading}
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 focus:ring-offset-slate-900 ${isSimulating ? 'bg-emerald-500' : 'bg-slate-700'} ${loading ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                >
                    <span
