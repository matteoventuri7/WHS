...
export class AppService {
  ...
  private async simulateDispatch() {
    this.logger.log('Checking vehicles ready for shipment...');

    // Genera randomicamente un nuovo camion (50% di probabilità)
    if (Math.random() < 0.5) {
      this.logger.log('Generating a new random truck...');
    ...
    try {
      ...
      if (!vehicles || !Array.isArray(vehicles)) {
        this.logger.warn('Invalid response format from shipping-service');
        return;
      }
      ...
      if (readyVehicles.length === 0) {
        this.logger.log(
          'No vehicles ready for dispatch (AVAILABLE with load > 0).',
        );
        return;
      }
      ...
      this.logger.log(
        `Ready vehicle found: ${vehicleToDispatch.vehicleId} (Load: ${vehicleToDispatch.currentLoad}/${vehicleToDispatch.maxCapacity})`,
      );
      ...
      try {
        ...
        this.logger.log(
          `Dispatch request successfully sent for vehicle ${vehicleToDispatch.vehicleId}.`,
        );
      } catch (dispatchError: any) {
        this.logger.error(
          `Error during vehicle dispatch ${vehicleToDispatch.vehicleId}: ${dispatchError.message}`,
        );
      }
    } catch (error: any) {
      this.logger.error(
        `Unable to contact shipping-service: ${error.message}`,
      );
    }
  }
  ...
  async generateTruck() {
    ...
    try {
      ...
      this.logger.log(
        `New truck generated by simulator: ${vehicleId} (Capacity: ${maxCapacity})`,
      );
      return response.data;
    } catch (error: any) {
      this.logger.error(
        `Error generating truck: ${error.message}`,
      );
      throw error;
    }
  }
