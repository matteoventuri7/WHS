...
describe('AppService', () => {
  ...
  describe('onModuleInit', () => {
    it('should log on init', async () => {
      ...
      expect(loggerSpy).toHaveBeenCalledWith('Connessione Kafka Producer per Shipping Service inizializzata.');
    });
  });
  ...
  describe('registerVehicle', () => {
    it('should save vehicle and emit events', async () => {
      ...
      expect(kafkaClient.emit).toHaveBeenCalledWith('VehicleRegistered', { vehicleId: 'V1', maxCapacity: 10 });
    ...
    it('should assign pending shipments if registered vehicle has capacity', async () => {
       const mockPending = { taskId: 'T1', orderId: 'O1', totalItems: 5, _id: '123' };
       mockQuery.sort.mockResolvedValueOnce([mockPending]); // from pending ship
       
       // the processPendingShipments accesses vehicleModel.find()
       const mockVehicle = new MockVehicleModel({ _id: '1', vehicleId: 'V1', maxCapacity: 10, currentLoad: 0, assignedTaskIds: [] });
       ...
       expect(kafkaClient.emit).toHaveBeenCalledWith('ShipmentAssigned', { taskId: 'T1', orderId: 'O1', vehicleId: 'V1' });
    });
  });

  describe('dispatchVehicle', () => {
    it('should dispatch successfully', async () => {
      const v = { vehicleId: 'V1', status: 'AVAILABLE', assignedTaskIds: ['T1'] };
      MockVehicleModel.findOneAndUpdate.mockResolvedValueOnce(v);

      const result = await service.dispatchVehicle('V1');
      expect(result).toEqual(v);
      expect(kafkaClient.emit).toHaveBeenCalledWith('VehicleDispatched', { vehicleId: 'V1', tasks: ['T1'] });
      expect(eventsGateway.notifyDataChanged).toHaveBeenCalled();
    });
    ...
    it('should throw an error if vehicle not found', async () => {
      ...
      await expect(service.dispatchVehicle('V2')).rejects.toThrow('Veicolo non trovato o non pronto');
    });
  });
  ...
  describe('handlePickingTaskCompleted', () => {
    it('should save as pending shipment if no vehicle available', async () => {
      ...
      const payload = { taskId: 'T1', orderId: 'O1', allocations: [{ quantity: 5 }] };
